        ...
        <view class="app-header">
            <view class="header-top">
                ...
                <view class="header-actions">
                    <u-button type="primary" size="mini" @click="submitArticle" :loading="submitLoading"
            ...
            <view class="form-container">
                ...
                <view class="form-section title-section">
                    ...
                    <view class="form-item">
                        <u--input v-model="articleForm.title" placeholder="请输入文章标题" border="surround"
                            class="title-input" />
                    </view>
                </view>
                ...
                <view class="form-section">
                    ...
                    <view class="form-item">
                        <u--textarea v-model="articleForm.summary" placeholder="请输入文章摘要" border="surround" auto-height
...
export default {
    ...
    computed: {
        // 表单验证
        isFormValid() {
            return this.articleForm.title.trim().length > 0 &&
    ...
    methods: {
        ...
        syncContentWithDisplay() {
            try {
                ...
                // 同时更新articleForm中的markdown内容
                this.articleForm.markdown = content;
            } catch (error) {
        ...
        async submitArticle() {
            ...
            try {
                ...
                const result = await this.createArticle(articleData);
                ...
                if (result && result.code === 200) {
                    ...
                    if (imageFiles.length > 0 && articleId) {
                        try {
                            // 逐个上传图片
                            for (let i = 0; i < imageFiles.length; i++) {
                                await new Promise((resolve, reject) => {
                                    uni.uploadFile({
                                        url: baseUrl + '/article/createArticleImage',
                            ...
                            console.error('文章图片上传错误:', uploadError);
                            uni.showToast({
                                title: '文章图片上传失败，请稍后重试',
                                icon: 'none'
                            });
                            return;
                        }
                    ...
                    setTimeout(() => {
                        ...
                        if (pages.length >= 2) {
                            ...
                            // 调用前一页的刷新方法（如果有的话）
                            if (prevPage && typeof prevPage.$vm.getRecommendArticles === 'function') {
                                prevPage.$vm.getRecommendArticles();
                            }
                            uni.navigateBack();
                        } else {
                            uni.switchTab({
                    ...
                    throw new Error(result.msg || '文章发布失败');
                }
            } catch (error) {
                console.error('文章发布失败:', error);
                uni.showToast({
                    title: error.message || '文章发布失败',
                    icon: 'none'
                });
            } finally {
                uni.hideLoading();
