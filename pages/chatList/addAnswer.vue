        ...
        <view class="app-header">
            <view class="header-top">
                ...
                <view class="header-actions">
                    <u-button type="primary" size="mini" @click="submitAnswer" :loading="submitLoading"
        ...
        <view class="content">
            <view class="form-container">
                ...
                <view class="form-section">
                    <text class="section-title">回答内容</text>
...
export default {
    ...
    computed: {
        // 表单验证
        isFormValid() {
            return this.submissionContent.trim().length > 0;
        },
    ...
    methods: {
        ...mapActions('taskAnswer', ['addTaskAnswer']),
        ...
        syncContentWithDisplay() {
            try {
                ...
                // 同时更新answerForm中的context内容
                this.answerForm.context = content;
            } catch (error) {
        ...
        async submitAnswer() {
            ...
            try {
                ...
                console.log('提交回答数据:', answerData);
                const result = await this.addTaskAnswer(answerData);

                if (result && result.code === 200) {
                    // 获取回答ID
                    const taskAnswerId = result.data;
                    
                    // 检查是否有本地图片文件需要上传
                    const imageFiles = this.imageMap.map(image => image.localPath).filter(path => path);
                    
                    // 如果有图片需要上传，直接使用UploadUtils上传
                    if (imageFiles.length > 0 && taskAnswerId) {
                        // 逐个上传图片
                        for (let i = 0; i < imageFiles.length; i++) {
                            await UploadUtils.uploadFile({
                                url: '/article/taskAnswer/createTaskAnswerImage',
                                filePath: imageFiles[i],
                                name: 'file',
                                formData: {
                                    answerId: taskAnswerId.toString()
                                }
                            });
                        }
                    }
                    ...
                    setTimeout(() => {
                        ...
                        if (pages.length >= 2) {
                            ...
                            // 调用前一页的刷新方法（如果有的话）
                            if (prevPage && typeof prevPage.$vm.fetchTaskAnswers === 'function') {
                                prevPage.$vm.fetchTaskAnswers();
                            }
                            uni.navigateBack();
                        } else {
                            uni.switchTab({
                    ...
                    throw new Error(result.msg || '回答发布失败');
                }
            } catch (error) {
                console.error('回答发布失败:', error);
                uni.showToast({
                    title: error.message || '回答发布失败',
                    icon: 'none'
                });
            } finally {
                uni.hideLoading();
