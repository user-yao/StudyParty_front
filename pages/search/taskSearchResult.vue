  ...
  <view class="search-result-container">
    ...
    <view class="content-container">
      ...
      <view class="section">
        ...
        <view class="article-list">
          ...
          >
            ...
            <view class="article-content">
              ...
              <view class="article-summary">{{ result.context }}</view>
            </view>
          ...
          <!-- 无结果提示 -->
          <view v-if="taskResults.length === 0 && searchCompleted" class="no-results">
        ...
        <!-- 加载更多提示 -->
        <view class="load-more" @click="loadMore" v-if="taskResults.length > 0 && !noMoreData">
          <text v-if="!loadingMore">点击加载更多</text>
          <text v-else>加载中...</text>
        </view>
        
        <!-- 无更多数据提示 -->
        <view class="no-more-data" v-if="taskResults.length > 0 && noMoreData">
          <text>没有更多数据了</text>
        </view>
      </view>
    </view>
...
export default {
  ...
  methods: {
    ...mapActions('article/task', ['searchTask', 'niceArticle', 'collectArticle']),
    ...
    async performSearch() {
      ...
      try {
        const res = await this.searchTask({ 
        ...
        if (res.code === 200) {
          ...
          // 处理搜索结果
          this.taskResults = res.data.records.map(item => ({
            ...item,
            type: 'task',
      ...
      try {
        ...
        if (res && res.code === 200) {
          ...
          if (res.data && Array.isArray(res.data.records) && res.data.records.length > 0) {
            ...
            this.taskResults = [...this.taskResults, ...moreResults];
    ...
    async toggleLike(task) {
      try {
        // 注意：这里可能需要调整，因为任务的点赞接口可能与文章不同
        const res = await this.niceArticle(task.id);
        if (res.code === 200) {
          if (res.data === "取消点赞") {
            task.isNice = false;
            task.nice -= 1;
    ...
    // 切换收藏状态
    async toggleCollect(task) {
      try {
        // 注意：这里可能需要调整，因为任务的收藏接口可能与文章不同
        const res = await this.collectArticle(task.id);
        if (res.code === 200) {
          if (res.data === "取消收藏") {
